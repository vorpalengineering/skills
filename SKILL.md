...
1. Install: `brew install vorpalengineering/tap/vorpal`
2. Get an API key from the Vorpal console (https://console.vorpalengineering.com → Access page)
3. Configure: `vorpal config set --api-key <key>`
...
```bash
vorpal knowledge search --json --limit 3 --threshold 0.3 "oracle price manipulation flash loan"
```
...
vorpal knowledge search --json "external call state update reentrancy"

# If reviewing access control
vorpal knowledge search --json "access control selfdestruct owner"

# Strict search for highly relevant results only
vorpal knowledge search --json --threshold 0.3 "flash loan oracle manipulation"
```
